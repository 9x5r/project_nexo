...
// Route pour recevoir l'XP envoyée par le DLL
app.get("/api/xp/update", async (req, res) => {
    const accountId = req.query.accountId;
    const xpToAdd = parseInt(req.query.xp);

    log.debug(`[XP UPDATE] Requête reçue pour ${accountId} avec ${xpToAdd} XP`);

    if (!accountId || isNaN(xpToAdd)) {
        return res.status(400).json({ error: "Données invalides" });
    }

    try {
        // On cherche l'utilisateur et on met à jour son XP
        // On utilise $inc pour ajouter l'XP à l'existante
        const user = await User.findOneAndUpdate(
            { accountId: accountId },
            { $inc: { xp: xpToAdd } }, // Assure-toi que ton modèle User a un champ 'xp'
            { new: true }
        );

        if (!user) {
            log.error(`[XP UPDATE] Utilisateur ${accountId} introuvable`);
            return res.status(404).json({ error: "Utilisateur non trouvé" });
        }

        log.debug(`[XP UPDATE] ${xpToAdd} XP ajoutés à ${user.username}. Nouveau total: ${user.xp}`);
        res.json({ status: "success", newTotal: user.xp });

    } catch (e) {
        log.error(`[XP UPDATE] Erreur: ${e.message}`);
...
app.get("/api/xp/update", async (req, res) => {
    ...
    if (!accountId || isNaN(xpToAdd)) return res.sendStatus(400);
    ...
    try {
        ...
        console.log(`[XP] +${xpToAdd} pour ${accountId}`);
        res.send("OK");
    } catch (e) {
