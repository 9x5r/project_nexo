...
function getContentPages(req) {
  ...
  try {
    if (
      ...
      const hasSpecialSection =
        contentpages.shopSections.sectionList.sections.some(
          (s) => s.sectionId === "Special",
        );
      ...
      // Ajouter dynamiquement les sections personnalisées depuis catalog_config.json
      try {
        const fs2 = require("fs");
        const path2 = require("path");
        const catalogConfigPath = path2.join(__dirname, "..", "Config", "catalog_config.json");
        if (fs2.existsSync(catalogConfigPath)) {
          const catalogConfig = JSON.parse(fs2.readFileSync(catalogConfigPath, "utf8"));
          const existingSectionIds = new Set(
            contentpages.shopSections.sectionList.sections.map(s => s.sectionId)
          );
          const knownSections = new Set(["featured", "daily", "special", "specia"]);

          for (const key in catalogConfig) {
            const kl = key.toLowerCase();
            const match = kl.match(/^([a-z]+)/);
            if (!match) continue;
            const prefix = match[1];
            if (knownSections.has(prefix)) continue;

            const sectionId = prefix.charAt(0).toUpperCase() + prefix.slice(1);
            if (existingSectionIds.has(sectionId)) continue;
            ...
            contentpages.shopSections.sectionList.sections.push({
              ...
              bShowIneligibleOffersIfGiftable: false,
              bEnableToastNotification: true,
              background: { stage: "default", _type: "DynamicBackground", key: "lobby" },
              _type: "ShopSection",
              landingPriority: 60,
              bHidden: false,
              sectionId: sectionId,
...
function getItemShop() {
  ...
  try {
    for (let value in CatalogConfig) {
      ...
      // Détecter dynamiquement la section depuis le nom de la clé
      // "daily..." → Daily | "featured..." → Featured | tout autre préfixe → section personnalisée
      const keyLower = value.toLowerCase();
      const isDaily = keyLower.startsWith("daily");
      const isFeatured = keyLower.startsWith("featured");

      // Extraire le nom de section: "janky1" → "Janky", "balenciaga2" → "Balenciaga"
      let sectionId = "Featured";
      if (isDaily) {
        sectionId = "Daily";
      } else if (!isFeatured) {
        const match = keyLower.match(/^([a-z]+)/);
        if (match && match[1]) {
          sectionId = match[1].charAt(0).toUpperCase() + match[1].slice(1);
        }
      }
      const tileSize = isDaily ? "Small" : "Normal";

      const CatalogEntry = {
        devName: "",
        offerId: "",
        fulfillmentIds: [],
        dailyLimit: isDaily ? 1 : -1,
        weeklyLimit: -1,
        monthlyLimit: -1,
        categories: [],
        prices: [
      ...
      // Daily → BRDailyStorefront | tout le reste → BRWeeklyStorefront
      let i = catalog.storefronts.findIndex(
        (p) => p.name == (isDaily ? "BRDailyStorefront" : "BRWeeklyStorefront"),
      );
      if (i == -1) continue;
